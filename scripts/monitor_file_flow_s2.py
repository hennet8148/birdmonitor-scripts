...
WATCH_DIR = "/Users/chuck/BirdMonitor/data/unprocessed_s2"

TIME_WINDOW_MINUTES = 10  # Adjust as needed
PHONE_NUMBER = "5708565105"  # AT&T number
SYNC_SCRIPT = "/Users/chuck/BirdMonitor/scripts/sync_recordings_s2.py"

# === Run sync_recordings_s2.py first ===
def run_sync():
    try:
        print(f"[{datetime.now()}] 🔄 Running sync_recordings_s2.py...")
...
def send_text_alert():
    message = f"⚠️ No new .wav files at Station 2 in the last {TIME_WINDOW_MINUTES} minutes."
...
def main():
    ...
    if last_seen is None or last_seen < threshold:
        ...
        print(f"[{datetime.now()}] ✅ Station 2: Last file seen at {datetime.fromtimestamp(last_seen)}")
